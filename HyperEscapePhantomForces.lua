...
ESPOptionsSection:CreateToggle("TeamCheck", false, function(ETC) esp.settings.TeamCheck = ETC; end);
	...
	if self and self.Character then
		...
		if tosro ~= nil then
			...
			if onScreen and IsAlive() and esp.settings.Enable then 
				...
				if esp.settings.TeamCheck ~= true or GetTeamColor(tosro) ~= localTeamColor() then
...
local function GetTarget(Hitscan, Fov, TeamCheck, WallCheck)
	...
	for i,Character in game.Workspace.Players:GetDescendants() do
		if Character.ClassName == "Model" then
			...
			if Head and Torso then
				if TeamCheck ~= true or GetTeamColor(Torso) ~= localTeamColor() then 
					...
					for i,HitParts in next, Hitscan do
						if HitParts == "Head" and HitParts == "Torso" then
							...
							if HeadScreenPos and HeadMagnitude < SmallestMagnitude and HeadMagnitude < Fov then
								if WallCheck ~= true or IsVisible(Head.Position, {Head.Parent, localPlayer.Character, game.Workspace.Ignore, currentCamera}) == true then
									SmallestMagnitude = HeadMagnitude;
									Target = Head;
								end
							end
							if TorsoScreenPos and TorsoMagnitude < SmallestMagnitude and TorsoMagnitude < Fov then
								if WallCheck ~= true or IsVisible(Torso.Position, {Head.Parent, localPlayer.Character, game.Workspace.Ignore, currentCamera}) == true then
									SmallestMagnitude = TorsoMagnitude;
									Target = Torso;
								end
							end
						elseif HitParts == "Torso" then
							local TorsoPos = Vector2.new(TorsoScreenPos.X, TorsoScreenPos.Y);
							local TorsoMagnitude = (TorsoPos - mouseLocation(UIS)).Magnitude;
							if TorsoScreenPos and TorsoMagnitude < SmallestMagnitude and TorsoMagnitude < Fov then
								if WallCheck ~= true or IsVisible(Torso.Position, {Head.Parent, localPlayer.Character, game.Workspace.Ignore, currentCamera}) == true then
...
game:GetService("RunService").Heartbeat:Connect(function() 
		...
		if HyperEscape.AimBot.IsAimKeyDown then
			if HyperEscape.AimBot.StickyAim then
				...
				if target ~= nil then -- This Is Here For Now As Idk Any Executors That Have Working mousemoverel
					--[[
					local Vector = currentCamera:WorldToScreenPoint(target.Position)
					local PosX = (localPlayer:GetMouse().X-Vector.X)/HyperEscape.AimBot.Smoothing + 1
					local PosY = (localPlayer:GetMouse().Y-Vector.Y)/HyperEscape.AimBot.Smoothing + 1
					mousemoverel(-PosX, -PosY)
					]]
				end
			end
		end
	...
	if HyperEscape.SilentAim.Enabled and IsAlive() then
		local target = GetTarget(HyperEscape.SilentAim.AimPart, HyperEscape.SilentAim.Fov, true, HyperEscape.SilentAim.WallCheck);
		local CamGun = GetGun();
